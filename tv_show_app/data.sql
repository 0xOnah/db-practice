...
-- joining table for reviewers and series
...
--Challange 4
-- Providing all data including the null values
SELECT * FROM series
LEFT JOIN reviews 
ON series.id = reviews.series_id;

--Showing fields with NULL
...
--Challange 6
--PostgresSql but will also work for MYSQL
SELECT first_name, last_name,
COUNT(rating) AS COUNT,
COALESCE(MIN(rating),0) AS MIN, COALESCE(MAX(rating),0) AS MAX,
ROUND(COALESCE(AVG(rating),0), 2) AS AVG,
CASE
...
--ANOTHER WAY FOR DOING SIMPLE CONDITIONALS IN MYSQL
